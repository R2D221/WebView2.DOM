...
public sealed class @html : HTMLHtmlElementBuilder, IEnumerable
{
	protected override HTMLHtmlElement CreateElement() =>
		document.createHTMLElement(HTMLElementTag.html);

	internal override bool CanAddChild(Node child)
	{
		return child is HTMLHeadElement
			|| child is HTMLBodyElement;
	}
	...
	public @html this[ChildNodes childNodes]
	{
		get
		{
	...
	{
		internal readonly List<Action<@html>> actions = new();
