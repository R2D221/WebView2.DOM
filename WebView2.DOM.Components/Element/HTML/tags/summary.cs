...
public sealed class @summary : HTMLElementBuilder, IEnumerable
{
	protected override HTMLElement CreateElement() =>
		document.createHTMLElement(HTMLElementTag.summary);

	internal override bool CanAddChild(Node child)
	{
		var extraChild = NodeExtraInfo.For(child);
		return extraChild.IsHeadingContent || extraChild.IsPhrasingContent;
	}
	...
	public @summary this[ChildNodes childNodes]
	{
		get
		{
	...
	{
		internal readonly List<Action<@summary>> actions = new();
