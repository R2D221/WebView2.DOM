...
public sealed class @ruby : HTMLElementBuilder, IEnumerable
{
	protected override HTMLElement CreateElement() =>
		document.createHTMLElement(HTMLElementTag.ruby);

	internal override bool CanAddChild(Node child)
	{
		var extraChild = NodeExtraInfo.For(child);
		return child is HTMLElement { tagName: "RUBY" or "RP" or "RT" }
	...
	public @ruby this[ChildNodes childNodes]
	{
		get
		{
	...
	{
		internal readonly List<Action<@ruby>> actions = new();
