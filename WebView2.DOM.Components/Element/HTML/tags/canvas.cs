...
public sealed class @canvas : HTMLCanvasElementBuilder, IEnumerable
{
	protected override HTMLCanvasElement CreateElement() =>
		document.createHTMLElement(HTMLElementTag.canvas);

	internal override bool CanAddChild(Node child)
	{
		var extraChild = NodeExtraInfo.For(child);

		return (extraChild.ContainsInteractiveContentExceptForAButtonInput == false);
	}
	...
	public @canvas this[ChildNodes childNodes]
	{
		get
		{
	...
	{
		internal readonly List<Action<@canvas>> actions = new();
