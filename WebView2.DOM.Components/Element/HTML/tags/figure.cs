...
public sealed class @figure : HTMLElementBuilder, IEnumerable
{
	protected override HTMLElement CreateElement() =>
		document.createHTMLElement(HTMLElementTag.figure);

	internal override bool CanAddChild(Node child)
	{
		var extraChild = NodeExtraInfo.For(child);
		return child is HTMLElement { tagName: "FIGCAPTION" }
	...
	public @figure this[ChildNodes childNodes]
	{
		get
		{
	...
	{
		internal readonly List<Action<@figure>> actions = new();
