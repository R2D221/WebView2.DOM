...
public sealed class @label : HTMLLabelElementBuilder, IEnumerable
{
	protected override HTMLLabelElement CreateElement() =>
		document.createHTMLElement(HTMLElementTag.label);

	internal override bool CanAddChild(Node child)
	{
		var extraChild = NodeExtraInfo.For(child);
		return extraChild.IsPhrasingContent
			&& (extraChild.ContainsLabel == false);
	}
	...
	public @label this[ChildNodes childNodes]
	{
		get
		{
	...
	{
		internal readonly List<Action<@label>> actions = new();
