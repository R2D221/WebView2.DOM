...
public sealed class @meter : HTMLMeterElementBuilder, IEnumerable
{
	protected override HTMLMeterElement CreateElement() =>
		document.createHTMLElement(HTMLElementTag.meter);

	internal override bool CanAddChild(Node child)
	{
		var extraChild = NodeExtraInfo.For(child);
		return extraChild.IsPhrasingContent
			&& (extraChild.ContainsMeter == false);
	}
	...
	public @meter this[ChildNodes childNodes]
	{
		get
		{
	...
	{
		internal readonly List<Action<@meter>> actions = new();
