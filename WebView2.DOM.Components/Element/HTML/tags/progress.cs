...
public sealed class @progress : HTMLProgressElementBuilder, IEnumerable
{
	protected override HTMLProgressElement CreateElement() =>
		document.createHTMLElement(HTMLElementTag.progress);

	internal override bool CanAddChild(Node child)
	{
		var extraChild = NodeExtraInfo.For(child);
		return extraChild.IsPhrasingContent
			&& (extraChild.ContainsProgress == false);
	}
	...
	public @progress this[ChildNodes childNodes]
	{
		get
		{
	...
	{
		internal readonly List<Action<@progress>> actions = new();
