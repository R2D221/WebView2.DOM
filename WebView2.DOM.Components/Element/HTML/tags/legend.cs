...
public sealed class @legend : HTMLLegendElementBuilder, IEnumerable
{
	protected override HTMLLegendElement CreateElement() =>
		document.createHTMLElement(HTMLElementTag.legend);

	internal override bool CanAddChild(Node child)
	{
		var extraChild = NodeExtraInfo.For(child);
		return extraChild.IsPhrasingContent
			|| extraChild.IsHeadingContent;
	}
	...
	public @legend this[ChildNodes childNodes]
	{
		get
		{
	...
	{
		internal readonly List<Action<@legend>> actions = new();
